export default class selection_screen2 extends Phaser.Scene {
  logo;
  musicBtn;
  music;
 isScenePaused = false;
...
soundManager.volume = 0.5;
      ...
      this.walk_snd = this.sound.add('walk_snd');
            ...
            this.monkey=this.add.image(-150,350,'monkey').setOrigin(0.5, 0.5).setScale(0.7);
            this.maskGraphics = this.add.graphics();
            this.maskGraphics.fillStyle(0xffffff,0);
            this.maskGraphics.fillRect(-10, 145, 700, 500);
            this.mask = this.maskGraphics.createGeometryMask();
            this.monkey.setMask(this.mask);


            this.sel_door2_1.on('pointerdown',function(){
            this.pick.play();
            this.arrow1.setVisible(false);
            this.sel_door2_1.disableInteractive();
            this.sel_door2_1.setTexture('sel_door2_2');
            this.tweens.add({targets:this.monkey,scaleX: 0.7,scaleY:0.7,ease: 'Quadratic',duration:700,delay:300,});
            this.walk_snd.play();
            this.tweens.add({targets:this.monkey,x:800,y:270,ease: 'Quadratic',duration:2000,delay:300,onComplete:() => {
            this.walk_snd.stop();
        ...
        this.tweens.add({targets:this.shutter_Bg,y:-252,ease: 'Quadratic',duration:1500,onComplete:() => {
            this.walk_snd.play();
        this.tweens.add({targets:this.monkey,x:450,ease: 'Quadratic',duration:2000,onComplete:() => {
            this.walk_snd.stop();
        this.sel_door2_1.setInteractive({ useHandCursor:true});
