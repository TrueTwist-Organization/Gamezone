...
// SafePointerLock.jspre
// Prevents pointer lock errors from crashing Unity WebGL in Chrome/Firefox
// when running in iframes (Poki, CrazyGames).
//
// Two error types can occur:
// - SecurityError: Chrome cooldown after Escape (~1.5s before re-lock allowed)
// - NotAllowedError: requestPointerLock() called without a valid user gesture
//
// Unity's loader.js registers a global "unhandledrejection" handler that shows
// a fatal alert() dialog on any unhandled Promise rejection, crashing the game.

(function() {
    function isPointerLockError(reason) {
        if (!reason || typeof reason.name !== 'string') return false;
        if (reason.name !== 'SecurityError' && reason.name !== 'NotAllowedError') return false;
        var msg = typeof reason.message === 'string' ? reason.message.toLowerCase() : '';
        return msg.indexOf('lock') !== -1 || msg.indexOf('pointer') !== -1 || msg.indexOf('gesture') !== -1;
    }

    // 1. Targeted unhandledrejection filter — runs BEFORE Unity's handler
    ...
    // 2. Monkey-patch requestPointerLock to catch rejections at the source
    var _original = Element.prototype.requestPointerLock;
    Element.prototype.requestPointerLock = function() {
        try {
            var result = _original.apply(this, arguments);
            if (result && typeof result.catch === 'function') {
                result = result.catch(function(err) {
                    if (err.name === 'SecurityError' || err.name === 'NotAllowedError') return;
                    throw err;
                });
            }
            return result;
        } catch (err) {
            if (err.name === 'SecurityError' || err.name === 'NotAllowedError') return Promise.resolve();
            throw err;
        }
    };
})();
...
const LoginApiBridge = {
  GetLoginTokenAsync(args) {
  },
  GetSecondaryLoginTokenAsync(args) {
  },
  InvalidateLoginToken(args) {
  },
  OpenLoginPage(args) {
  },
  Logout(args) {
...
Module['MetaplayApiBridge']['LoginApiBridge'] = LoginApiBridge;
// This file is part of Metaplay SDK which is released under the Metaplay SDK License.
...
(function(h,p){typeof exports=="object"&&typeof module<"u"?p(exports):typeof define=="function"&&define.amd?define(["exports"],p):(h=typeof globalThis<"u"?globalThis:h||self,p(h["metaplay-browser-sdk"]={}))})(this,(function(h){"use strict";const p="metaplay_oauth2_login_flow.version1";function G(e){return typeof e=="object"&&e!==null&&"access_token"in e}function H(e){return typeof e=="object"&&e!==null&&"error"in e}function k(e){return"error"in e}function J(e,i,r,a,c){const{verifier:s,challenge:d}=x(a),l=I(!1,""),g=j({verifier:s,host:e.toString(),clientId:i,redirectUri:l,authPlatform:r,secondaryAuthentication:c}),f=E({gameServerUrl:e,clientId:i,authPlatform:r,redirectUri:l,stateId:g,challenge:d});window.location.href=f}function K(e,i,r,a,c,s,d,l){const{verifier:_,challenge:L}=x(a),S=I(!0,s),v=j({verifier:_,host:e.toString(),clientId:i,redirectUri:S,authPlatform:r,secondaryAuthentication:c}),U=E({gameServerUrl:e,clientId:i,authPlatform:r,redirectUri:S,stateId:v,challenge:L}),y=(screen.width-(d??400))/2,A=(screen.height-(l??520))/2;window.open(U,"loginPopupWindow",`modal=yes, left=${y}, top=${A}, width=${d??400}, height=${l??520}, toolbar=no, resizable=no`)}async function O(e,i){let r;try{r=await fetch(new URL("auth/token",e),{method:"POST",mode:"cors",body:i})}catch{return{error:"Network error",description:"unknown",isTransient:!0}}if(!r.ok&&r.status!==400)return{error:"Query failed with status code "+r.status,description:await r.text(),isTransient:!0};const a=await r.json();return H(a)?{error:a.error,description:a.error_description,isTransient:!1}:G(a)?a:{error:"Malformed response from token endpoint",description:"",isTransient:!1}}async function V(e){const i=e.get("state"),r=e.get("code"),a=e.get("error"),c=e.get("error_description");if(!X(i)||!r)return{error:"",description:"",isTransient:!0};if(a)return z(),{error:a,description:c??"",isTransient:!1};const s=z();if(!s)throw Error("Fatal error in retrieving client flow state");const d=new URL(s.host),l=new URLSearchParams;l.append("grant_type","authorization_code"),l.append("client_id",s.clientId),l.append("redirect_uri",s.redirectUri),l.append("code",r),l.append("code_verifier",s.verifier);const g=await O(d,l);if(k(g))return g;const f=new URLSearchParams(e);return f.delete("oauth_callback"),f.delete("state"),f.delete("code"),f.delete("error"),f.delete("error_description"),{token:g,authPlatform:s.authPlatform,gameServerUrl:d,secondaryAuthentication:s.secondaryAuthentication,newQuery:f}}async function Q(e,i){const r=new URLSearchParams;return r.append("grant_type","refresh_token"),r.append("refresh_token",i),r.append("scope","game.session"),await O(e,r)}function C(e=8){const i=new Uint8Array(e);return crypto.getRandomValues(i),M(i)}function E(e){const i=new URL(`auth/login/${e.authPlatform}`,e.gameServerUrl),r=i.searchParams;return r.append("client_id",e.clientId),r.append("redirect_uri",e.redirectUri),r.append("state",e.stateId),r.append("response_type","code"),r.append("code_challenge_method","S256"),r.append("code_challenge",e.challenge),r.append("scope","game.session"),i.toString()}function I(e,i){const r=new URL(location.href);return r.hash="",r.search="?oauth_callback=true",e&&(r.pathname=i),r.toString()}function M(e){const i=Array.from(e,r=>String.fromCodePoint(r)).join("");return btoa(i).replaceAll("=","").replaceAll("+","-").replaceAll("/","_")}async function T(){if(!window.isSecureContext)return{needSecureContextError:!0,verifier:"",challenge:""};const e=C(32),i=new TextEncoder().encode(e),r=await crypto.subtle.digest("SHA-256",i),a=M(new Uint8Array(r));return{needSecureContextError:!1,verifier:e,challenge:a}}function x(e){if(e.needSecureContextError)throw new Error("Login can only be completed in a Secure Context. This page is not secure.");return{verifier:e.verifier,challenge:e.challenge}}function j(e){const i=C(8);return sessionStorage.setItem(p,i),sessionStorage.setItem(p+"_state",JSON.stringify(e)),i}function Y(){sessionStorage.removeItem(p),sessionStorage.removeItem(p+"_state")}function X(e){if(!e)return!1;const i=sessionStorage.getItem(p);return i?e===i:!1}function Z(e){return typeof e=="object"&&e!==null&&"verifier"in e}function z(){const e=sessionStorage.getItem(p+"_state");if(Y(),!e)return null;const i=JSON.parse(e);return Z(i)?i:null}async function q(e,i,r,a){if(!("MetaplayApiBridge"in e.Module))throw Error("MetaplayApiBridge module not found in Unity runtime.");const c=e.Module.MetaplayApiBridge;if(c.MetaplayApp)throw Error("MetaplayApp already exists in the Unity runtime. Duplicate initialization?");if(c.IsInitialized)throw Error("MetaplayApiBridge already initialized in Unity runtime. Duplicate initialization?");if(c.MetaplayApp={args:{...i,loginApiBridgeVersion:1}},c.LoginApiBridge=r,a)for(const s in a)Object.hasOwn(a,s)&&(c[s]=a[s]);if(await new Promise((s,d)=>{c.MetaplayApp&&(c.MetaplayApp.OnInitialized=s)}),!c.IsInitialized)throw Error("Unity runtime bridge failed to initialize.");return c}function ee(e){return typeof e=="object"&&e!==null&&"authPlatform"in e}function B(e){return typeof e=="object"&&e!==null&&"type"in e}function te(e="UnityWebGL",i="closePopup",r=void 0){const a={};let c=null,s;const d=oe();let l=null,g=!0;$();async function f(t,o){let n=null,u=null;const w=t.data;if(typeof w=="string"?n=/Metaplay:LoginCompleted:(.+)/.exec(w)?.[1]:B(w)&&(n=w.data,u=w.token),n===null)return;const R=new URLSearchParams(n),b=await P(R,!0);k(b)?console.log(`(Popup) Token exchange failed: ${b.error} -- ${b.description}`):console.log(`(Popup) Token exchange completed: ${b.token.access_token}`),o&&u&&o.postMessage({type:"ack",token:u}),L()}function _(){const t=new BroadcastChannel("Metaplay:LoginCompleted"),o=u=>{f(u,null)},n=u=>{f(u,t)};return window.addEventListener("message",o),t.addEventListener("message",n),{broadcastChannel:t,windowCallback:o,broadcastCallback:n}}function L(){l&&(window.removeEventListener("message",l.windowCallback),l.broadcastChannel.removeEventListener("message",l.broadcastCallback),l=null)}function S(t){return t.hostname}function v(t){return`GameServerAccessTokens/${e}/${t}`}function U(t){const o=localStorage.getItem(v(t));if(!o)return;const n=JSON.parse(o);return ee(n)?n:void 0}function y(t){const o=S(t);if(o in a)return a.key;const n=U(o);return n&&(a.key=n),n}function A(t,o){return{authPlatform:o,accessToken:t.access_token,expiresAt:new Date(Date.now()+t.expires_in*1e3).toISOString(),refreshToken:t.refresh_token}}function m(t,o){const n=S(o);t?(localStorage.setItem(v(n),JSON.stringify(t)),a.key=t):(localStorage.removeItem(v(n)),delete a.key),r?.(o,t)}async function D(){if(!g)return;const t=new URLSearchParams(window.location.search);t.has("oauth_callback")&&(await P(t,!1),g=!1)}function oe(){return{AccountChanged:void 0,SecondaryAuthenticationCompleted:void 0,async GetLoginTokenAsync(t){await D();const o=new URL(t.gameServerUrl);let n=y(o);if(!n)return{status:"notLoggedIn"};if(!n.accessToken||new Date(n.expiresAt)<new Date){if(n=await F(o),!n)return{status:"mustRelogin"};if(!n.accessToken)return{status:"refreshError"}}return{status:"ok",token:n.accessToken}},async GetSecondaryLoginTokenAsync(t){await D();let o;return c&&(o=c.accessToken??void 0,c=null),{status:o?"ok":"notLoggedIn",token:o}},InvalidateLoginToken(t){const o=new URL(t.gameServerUrl),n=y(o);n&&m({...n,expiresAt:new Date().toISOString(),accessToken:null},o)},OpenLoginPage(t){const o=new URL(t.gameServerUrl);t.usePopup?N(o,t.platform,t.secondaryAuthentication):W(o,t.platform,t.secondaryAuthentication)},Logout(t){const o=new URL(t.gameServerUrl);m(void 0,o)}}}function re(){if(s!==null){const t=s;return s=null,$(),new Promise((o,n)=>{o(t)})}return T()}function $(){T().then(t=>{s=t})}async function P(t,o){const n=await V(t);if(k(n))return n;const u=A(n.token,n.authPlatform);return n.secondaryAuthentication?(c=u,o&&d.SecondaryAuthenticationCompleted?.("")):(m(u,n.gameServerUrl),o&&d.AccountChanged?.("")),n}async function F(t){const o=y(t);if(!o){console.debug("not logged in, nothing to refresh");return}const n=await Q(t,o.refreshToken);if(!k(n)){const u=A(n,o.authPlatform);return m(u,t),u}if(n.isTransient)return console.debug("transient error in token refresh: "+n.error),o;console.debug("non-transient error in token refresh, clearing tokens: "+n.error),m(void 0,t)}async function W(t,o,n){const u=await T();J(t,e,o,u,n)}async function N(t,o,n,u,w){const R=await re();l??=_(),K(t,e,o,R,n,i,u,w)}function ie(t){m(void 0,t),d.AccountChanged?.("")}return{getToken:y,refreshTokens:F,handleOauthCallbackAsync:P,loginWithRedirect:W,loginWithPopup:N,logout:ie,loginApiBridge:d,deinitialize:()=>{L()},initializeUnityRuntimeBridge:async function(t,o,n){g=!1,await q(t,o,d,n)}}}function ne(){const e=new URLSearchParams(window.location.search);if(window.opener)window.opener.postMessage(`Metaplay:LoginCompleted:${e}`),window.close();else{const i=new BroadcastChannel("Metaplay:LoginCompleted"),r=C(8);i.addEventListener("message",a=>{B(a.data)&&a.data.type==="ack"&&a.data.token===r&&window.close()}),i.postMessage({type:"notify",data:e.toString(),token:r})}}h.handleOauthPopupCallback=ne,h.initializeBrowserSdkClient=te,h.isTokenError=k,Object.defineProperty(h,Symbol.toStringTag,{value:"Module"})}));
/*
CryptoJS v3.1.2
...
class WebApiBridgeRegistry {
  constructor(metaplayApiBridge, invokeSyncMethod, invokeAsyncMethod, bridgeClasses) {
    const self = this;
    self.metaplayApiBridge = metaplayApiBridge;
        ...
        if (methodInfo.isAsync) {
          browserModule[methodInfo.methodName] = function asyncMethodWrapper(args) {
            return new Promise((resolve, reject) => {
              ...
              try {
                invokeAsyncMethod(reqId, methodInfo.methodId, argsBuffer)
              } finally {
                _free(argsBuffer);
              }
            });
          ...
          browserModule[methodInfo.methodName] = function syncMethodWrapper(args) {
            ...
            try {
              if (methodInfo.hasReturnValue) {
                const replyPtr = invokeSyncMethod(methodInfo.methodId, argsBuffer)
                try {
                ...
                invokeSyncMethod(methodInfo.methodId, argsBuffer)
                return undefined;
              }
            } finally {
              _free(argsBuffer);
  ...
  deinitialize() {
    const self = this;
    for (const classInfo of self.bridgeClasses) {
      const browserModule = self.metaplayApiBridge[classInfo.className];
      for (const methodInfo of classInfo.exportedMethods) {
        browserModule[methodInfo.methodName] = null
      }
    }
  }
};
    ...
    if (typeof getWasmTableEntry !== "undefined") {
      return function (reqId, reply, errorStr) {
        if (errorStr === null) {
          ...
          try {
            getWasmTableEntry(callbackMethod)(reqId, replyBuffer, null)
          } catch (err) {
          ...
          try {
            getWasmTableEntry(callbackMethod)(reqId, null, errorBuffer)
          } catch (err) {
